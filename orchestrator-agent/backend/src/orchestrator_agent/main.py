...
from orchestrator_agent.a2a.card import build_agent_card
from orchestrator_agent.a2a.executor import OrchestratorAgentExecutor
from orchestrator_agent.api.routes import router as api_router
...
def _spa_index(static_dir: Path) -> Path:
    ...
    if not index.is_file():
        raise HTTPException(
            status_code=503,
            detail="UI not built. From orchestrator-agent/ui run: npm install && npm run build",
        )
    return index
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        ...
        description=(
            ...
            "POST /ingest (or A2A) accepts an architect design package; continue on the shared UI."
        ),
        version="0.1.0",
    )
...
def main() -> None:
    settings = get_settings()
    logger.info("Starting orchestrator-agent on %s:%s provider=%s", settings.host, settings.port, settings.llm_provider)
    uvicorn.run(
        "orchestrator_agent.main:app",
        host=settings.host,
        port=settings.port,
        reload=False,
    )
