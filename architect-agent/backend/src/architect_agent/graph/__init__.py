...
) -> Literal["phase0_classify", "lld_step", "hld_step", "__end__"]:
...
def _after_lld_wait(
    state: DesignGraphState,
) -> Literal["lld_step", "market_research", "__end__"]:
    if state.get("phase") == "done":
        return "__end__"
...
def _after_hld_wait(
    state: DesignGraphState,
) -> Literal["hld_step", "market_research", "__end__"]:
    if state.get("phase") == "done":
        return "__end__"
    if state.get("phase") == "market_research":
        return "market_research"
