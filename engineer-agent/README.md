...
the plan in the UI to start a long-running coding loop (priority order, peer contracts
for cross-service items). Pause to revise; execute again to transition from current
progress. Code is shipped to git only after the entire plan completes. The sub-engineer
pulls the fleet repo first and works in a private folder at the git root named after
the microservice.
